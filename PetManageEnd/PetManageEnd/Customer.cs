    ...
    public partial class Customer : Form
    {
        ...
        private void dgvCustomer_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            else if (colName == "Delete")
            {
                ...
                {
                    dbcon.excuteQuery("DELETE FROM tbCus WHERE id LIKE '" + dgvCustomer.Rows[e.RowIndex].Cells[1].Value.ToString() + "'");
        ...
        public void LoadCustomer()
        {
            ...
            cm = new SqlCommand("SELECT * FROM tbCus WHERE CONCAT(name, address, phone) LIKE '%" + txtSearch.Text + "%'", cn);
            cn.Open();
            dr = cm.ExecuteReader();
            while (dr.Read())
            {
